...
public class Main {
    public static void main(String[] args) {
        ...
        long baseTime = System.currentTimeMillis(); // фиксируем старт программы

        for (Animal pet : pets) {
            futures.add(executor.submit(() -> {
                long startTimeMillis = System.currentTimeMillis() - baseTime; // время старта относительно начала программы
                double walkTime = pet.goToWalk(); // время прогулки
                ...
                long endTimeMillis = System.currentTimeMillis() - baseTime; // время окончания относительно старта программы
        ...
        // Дожидаемся выполнения всех потоков и собираем результаты
        futures.forEach(f -> {
            try {
                System.out.println(f.get()); // Выводим информацию в порядке завершения потоков
